        ...
        <form action="" method="post">
            ...
            <div class="receipt-comand">
                ...
                <div class="comand--btn">
                    <button type="submit" name="gh">xác nhận pha chế</button>
                </div>
            </div>
            ...
            <table class="receipt-table" >
                ...
                FROM hoadon JOIN khachhang on khachhang.idKH = hoadon.IdKhachHang JOIN  diachi on diachi.IDKhachHang = khachhang.idKH WHERE hoadon.TrangThai = '1' ";
                $result = mysqli_query($conn, $query);
                $numRow = mysqli_num_rows($result);
                $maxPage = ceil($numRow / $rowPerPage); 
                $query = "SELECT hoadon.IdHoaDon,diachi.DiaChi, khachhang.TenKH,khachhang.GioiTinh,khachhang.Sdt, khachhang.idKH,hoadon.TrangThai,hoadon.NgayDat,hoadon.TongTien
                FROM hoadon JOIN khachhang on khachhang.idKH = hoadon.IdKhachHang JOIN  diachi on diachi.IDKhachHang = khachhang.idKH WHERE hoadon.TrangThai = '1'  LIMIT $offset, $rowPerPage";
                $result =  mysqli_query($conn, $query);
                if($result){
                    if(mysqli_num_rows($result) != 0)
                    {
                        while($row = mysqli_fetch_array($result))
                        {
                            ...
                            echo "<td>";
                            if($row["TrangThai"] == 1)
                                echo '<span class="paid">đang đặt</span>';
